...
MgSession.prototype.selectTerm = function() {
  var deferred = q.defer();

  this.login()
  .then(this.promiseTermPage)
  .then(function(promised_obj) {
    deferred.resolve(promised_obj.body);
  }, function(err) {
    deferred.reject(err);
  }).done();
...
MgSession.prototype.promiseTermPage = function(promised_obj) {
  var deferred = q.defer();

  var url= CONSTANTS.URLS.select_term;
  var jar= promised_obj.jar;
  var form= {
  ...
  .then(function (promised_obj) {
    if (MgSession.isLoggedIn(promised_obj.body)) deferred.resolve(promised_obj);
    else deferred.reject(new Error('couldnt resolve term selection'));
  }, function(err) {
    deferred.reject(err);
  });
...
MgSession.prototype.selectCourses = function(selection) {
  var deferred = q.defer();
